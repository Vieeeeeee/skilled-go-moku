...
const skills: Skill[] = [
  { id: 'remove', name: '飞沙走石', cost: 5, description: "移除棋盘上对手的一枚棋子。" },
  { id: 'skip', name: '静如止水', cost: 7, description: '让对手的回合停滞，跳过其下一回合。' },
  { id: 'swap', name: '两极反转', cost: 15, description: '交换棋盘上所有的黑白棋子。' },
  { id: 'tiaoChengLiShan', name: '调呈离山', cost: 8, description: '移动对手一枚棋子到任意空位。' },
  { id: 'qinNa', name: '擒拿', cost: 12, description: '替对手下一轮棋子' },
  { id: 'cleaning', name: '保洁上门', cost: 10, description: '随机清除对手1-3枚棋子，棋盘焕然一新！' },
  { id: 'overwhelm', name: '力拔山兮', cost: 20, description: '发动此最终奥义，无视规则，直接获得此战的胜利。'},
];
...
const App: React.FC = () => {
  ...
  const handleUltimateSkill = () => {
    ...
    // Create ultimate skill object
    const ultimateSkill: Skill = {
      id: 'overwhelm', // Reuse overwhelm for animation theme
      name: '终极奥义',
      cost: 0,
  ...
  return (
    <div className={`min-h-screen flex flex-col p-4 relative ${isCelebrating ? 'celebration-active' : ''} overflow-hidden`}>
      ...
      {/* Skill Video Player */}
      {skillVideo && (
        <div className={`fixed z-40 ${skillVideo.includes('终极奥义') ? 'inset-0 flex items-center justify-center bg-black/70 backdrop-blur-sm' : 'bottom-10 right-10'} skill-video-container`}>
          <div className="relative rounded-xl overflow-hidden shadow-2xl border-4 border-amber-500 bg-black">
            <video
              ...
              className="h-auto"
              style={{ width: skillVideo.includes('终极奥义') ? '50vw' : '40vw', maxWidth: skillVideo.includes('终极奥义') ? '960px' : '800px', minWidth: skillVideo.includes('终极奥义') ? '600px' : '500px' }}
            />
            <button
              onClick={() => {
                setSkillVideo(null);
